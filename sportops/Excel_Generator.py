...
def build_total_sports_total_sheet(ws, title, sports, num_items, ifclient):
    ...
    # Column headers
    headers = ["Institution"] + sports + ["Total Operating Expenses"]
    ...
    for col in range(3, 2 + len(sports) + 2):
        if col in (3, 4, 5, 6, 7, 9, 11, 12, 13, 14):
            ws.column_dimensions[get_column_letter(col)].width = 12.5
        elif col == 2 + len(sports) + 1:
    ...
    for r, label in [(avg_row, "Average"), (median_row, "Median"), (rank_row, "Rank")]:
        ...
        for col in range(2, 3 + len(sports) + 1):  # metrics columns
...
def build_totalsports_sheet(ws, title, num_items,ifclient):
    ...
    # Column headers
    headers = ["Institution"] + metrics + ["Total Operating Expenses"]
    for idx, text in enumerate(headers, start=2):  # B=2
        cell3 = ws.cell(row=3, column=idx, value=text)
...
def generate_template_excel_totalsports(output_path):
    wb = Workbook()

    '''    # Men’s sheet

    for idx, title in enumerate(ws_title_list):
        if idx == 0:
            ws = wb.active  # use the existing default sheet for the first one
        else:
            ws = wb.create_sheet()  # create new sheet for all others
        ws.title = title
        build_totalsports_sheet(
            ws,
            f"CONFERENCE - {client_name} Sport Operating Budgets",
            num_items
        )'''
